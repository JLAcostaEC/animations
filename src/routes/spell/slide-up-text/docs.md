...
<SlideUpText
  text="Copy arrives with a clean upward stagger."
  class="max-w-[16ch] text-4xl font-semibold tracking-tight"
/>
```
...
<SlideUpText text="Signals stay clear" split="characters" from="center" />
```

## View Trigger

Set `inView` to wait for viewport entry. Use `once={false}` if you want the reveal to reset after leaving the viewport.
...
<SlideUpText text="Timed to the viewport" inView once={false} />
```
...
| ------------ | ------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------- |
| `text`       | `string`                             | required                                      | Plain text content that is split into animated words, characters, or lines. |
