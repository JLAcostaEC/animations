<script lang="ts">
	...
	const letterSpacingValue = $derived.by(() => {
	...
	const tokens = $derived.by<RenderToken[]>(() => {
		...
		for (const [tokenIndex, token] of entries.entries()) {
			...
			const characters = splitIntoCharacters(token).map((value, characterIndex) => ({
				value,
